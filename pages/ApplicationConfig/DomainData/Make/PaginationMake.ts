...
export class DomainDataPagination extends BasePage {
  page: Page;
  ...
  public async verifyDomainDataPagination(testInfo: TestInfo): Promise<void> {
    const options = await this.pageSizeDropdown.locator("option").evaluateAll(options => options.map(option => ({ text: (option as HTMLOptionElement).text, value: (option as HTMLOptionElement).value })));
    for (const option of options) {
      ...
      Reporter.validateData(requestedValue, effectivePageSize, `Dropdown Selection (${option.text})`, testInfo);
  ...
  private async getPaginationInfo(): Promise<PaginationInfo> {
    ...
    const match = paginationText.match(/Showing\s+(\d+)[-\s]+(\d+)\s+of\s+(\d+)/i);
    if (!match) {
