...
export class YearPagination extends BasePage {
  ...
  constructor(page: Page) {
    ...
    // COLOR TABLE
    // ==================================================
    this.colorTable = page.locator(
      'table:has(th:has-text("Color"))'
    );
  ...
  // ==================================================
  public async verifyyearPagination(
    testInfo: TestInfo
  ): Promise<void> {
  ...
  private async testPaginationNavigation(
    ...
    for (let pageNum = 2; pageNum <= maxPagesToTest; pageNum++) {
      ...
      logAndValidate({ step: `Navigation to Page ${pageNum}`, expected: pageNum, actual: currentInfo.currentPage }, testInfo);

      const expectedStartRecord = (pageNum - 1) * pageSize + 1;
      const expectedEndRecord = Math.min(pageNum * pageSize, initialInfo.totalRecords);

      logAndValidate({ step: `Start Record for Page ${pageNum}`, expected: expectedStartRecord, actual: currentInfo.startRecord }, testInfo);
      logAndValidate({ step: `End Record for Page ${pageNum}`, expected: expectedEndRecord, actual: currentInfo.endRecord }, testInfo);

      const actualRows = await this.tableRows.count();
      const expectedRowsOnPage = currentInfo.endRecord - currentInfo.startRecord + 1;
      logAndValidate({ step: `Rows on Page ${pageNum}`, expected: expectedRowsOnPage, actual: actualRows }, testInfo);

      pageData = await this.collectAndValidatePageData(pageNum, testInfo);
      pageData.recordIds.forEach(id => { if (!allRecordIds.has(id)) allRecordIds.add(id); });
    }
    ...
    if (isFirstPageEnabled) {
      ...
      await this.waitForTableToUpdate();
      const currentInfo = await this.getPaginationInfo();
      logAndValidate({ step: `Return to Page 1`, expected: 1, actual: currentInfo.currentPage }, testInfo);
    }
    ...
    if (isLastPageEnabled && initialInfo.totalPages > 1) {
      ...
      await this.waitForTableToUpdate();
      const currentInfo = await this.getPaginationInfo();
      logAndValidate({ step: `Last Page Navigation`, expected: initialInfo.totalPages, actual: currentInfo.currentPage }, testInfo);
      const actualRows = await this.tableRows.count();
      ...
      logAndValidate({ step: `Last Page Row Count`, expected: expectedRowsOnLastPage, actual: actualRows }, testInfo);
      await this.navigateToFirstPage();
    }
  }
  ...
  private async waitForTableToUpdate(): Promise<void> {
    try {
      await this.colorTable.locator("tbody").waitFor({ state: "attached", timeout: 10000 });
      await this.colorTable.locator("tbody tr").first().waitFor({ state: "visible", timeout: 10000 });
