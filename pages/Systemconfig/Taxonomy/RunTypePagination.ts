...
export class RunTypePagination {
...
this.paginationText = page.locator('text=/Showing .* of .* entries/i');
}
...
await this.paginationText.first().waitFor({ state: 'visible', timeout: 10000 });

const text = await this.paginationText.first().textContent();
...
await this.page.waitForFunction(() => document.body.innerText.includes('Showing 1-'));
...
const isEnabled = isVisible ? await this.nextButton.isEnabled() : false;
...
return row && row.textContent !== oldValue;
...
const counts = await this.showCountDropdown
...
console.log(`🔍 Testing RunType Pagination: ${value}`);
...
await this.report(`Next Button (${value})`, true, nextWorked);

await this.report(`Total Records (${value})`, expectedTotal, actualTotal);
