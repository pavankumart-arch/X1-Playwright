...
import { RooftopPermission } from '../../../../pages/Rooftops/edit_delete_Permission';
import AddRooftopData from '../../../../testdata/AddRooftopData.json';
import { RooftopNavigation } from '../../../../pages/Rooftops/RooftopNavigation';
;

test("Verify Reseller Admin Left Navigation", async ({ page }) => {
    ...
    console.log("\nVerify Users");
    console.log("Expected : Users should not be visible");

    if (await navigation.Users.count() == 0) {
        console.log(`${GREEN}Status : PASS${RESET}`);
        console.log("Actual : Users is not visible. Working as expected.");
    } else {
        console.log(`${RED}Status : FAIL${RESET}`);
...
const InventoryNavigation = new RooftopNavigation(page);
  ...
  await page.waitForLoadState('networkidle');   
    ...
    const rooftopPermission=new RooftopPermission(page);
    rooftopPermission.verifyDeleteButtonNotAvailable("Test Rooftop",test.info());
    rooftopPermission.verifyEditButtonNotAvailable("Test Rooftop",test.info());
