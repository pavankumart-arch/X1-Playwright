...
test.describe('Verify the rooftop Delete User functionality', () => {

  test('Verify that a rooftopuser can be deleted successfully', 
    async ({ page }, testInfo) => {
 ...
 // Step 1 & Step 2
  ...
  await page.waitForLoadState('networkidle');
  const RooftopdeleteUser = new rooftopdeleteUser(page);
  await RooftopdeleteUser.DeleterooftopUser(testInfo.title);
